...
public class LaserMeshGenerator : MonoBehaviour
{
	...
	private void CreateLaserInstance()
	{
		...
		renderer.materials = laserMaterials;
	...
	private void CreateNewMeshPoints()
	{
		...
		var lastIndex = currentMeshInfo.points.Count - 1;
		var lastPoint = currentMeshInfo.points[lastIndex];
		var midPoint = new Point((lastPoint.direction + newPoint.direction) / 2f, (lastPoint.position + newPoint.position) / 2f, (lastPoint.speed + newPoint.speed) / 2f, laserintervalSpeed);
		currentMeshInfo.points.Add(midPoint);
