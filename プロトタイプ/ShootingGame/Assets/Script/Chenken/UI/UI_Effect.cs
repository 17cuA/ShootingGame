...
public class UI_Effect : UI_EffectBase
{
	//################################
...
#pragma warning disable 0414
	...
	[Obsolete]
	[HideInInspector]
	[SerializeField] List<UI_Shadow.AdditionalShadow> m_AdditionalShadows = new List<UI_Shadow.AdditionalShadow>();
	...
	protected override void UpgradeIfNeeded()
	{
		...
		if (IsShouldUpgrade(300))
		{
			...
			if (m_ShadowStyle != ShadowStyle.None || m_AdditionalShadows.Any(x => x.style != ShadowStyle.None))
			{
				...
				{
					var shadow = gameObject.GetComponent<UI_Shadow>() ?? gameObject.AddComponent<UI_Shadow>();
				...
				foreach (var s in m_AdditionalShadows)
				{
					...
					var shadow = gameObject.AddComponent<UI_Shadow>();
			...
			if (tone == Hue)
			{
				...
				DestroyImmediate(this, true);
				var hsv = go.GetComponent<UI_HsvModifier>() ?? go.AddComponent<UI_HsvModifier>();
				hsv.hue = hue;
				hsv.range = 1;
			}
			...
			if (tone == Cutoff || tone == Mono)
			{
				...
				var transitionMode = tone == Cutoff
					? UI_TransitionEffect.EffectMode.Cutoff
					: UI_TransitionEffect.EffectMode.Fade;
				DestroyImmediate(this, true);
				var trans = go.GetComponent<UI_TransitionEffect>() ?? go.AddComponent<UI_TransitionEffect>();
