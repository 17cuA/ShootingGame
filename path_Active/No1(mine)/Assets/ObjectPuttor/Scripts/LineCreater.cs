...
//────────────────────────────────────────────
// 更新履歴：
// 2019/03/08 [久保田] クラス作成。LineRendererを使い線をつないでいく
// 2019/03/08 [久保田] 実行中ではなくエディター上で動くようにする
// 2019/03/08 [久保田] マウスの左クリックでオブジェクトを置けるように修正　←撤廃。editモードでマウス入力が受け付けられない。
// 2019/03/11 [久保田] マウスの左クリックでオブジェクトを置くための機能を発見。実装を行う。
// 2019/03/11 [久保田] これ以降の実装のためのタスクが多いためマウスでの操作は一旦保留する。
// 2019/04/16 [久保田] 4点以上置かれたときのプログラムを作成
// 2019/04/16 [久保田] 二次ベジェ曲線から三次ベジェ曲線に修正
// 2019/04/16 [久保田] 新たに点が置かれたときに、ハンドルを生成させる
// 2019/04/19 [久保田] アンカークラスを組み込む。
// 2019/05/08 [久保田] 親子関係の中で新たにアンカーが追加されたとき、自動的に追加する。
// 2019/05/09 [久保田] 次のアンカーが生成されたとき、自動的にいい感じの角度に変える
// 2019/05/09 [久保田] クリックしたときクリックされた場所にアンカーを配置する。
// 2019/05/09 [久保田] 実行と同時に一定区間ごとにオブジェクトを置く
// 2019/05/09 [久保田] アタッチされているはずのものが存在しない場合、警告文を出す
// 2019/05/13 [久保田] エディタ上のみで更新処理を行い、それ以外では処理を行わないようにする
// 2019/05/13 [久保田] 角度調整機能を一部調整
// 2019/05/13 [久保田] 長さに応じて間隔を自動調整する(失敗した)
// 2019/05/22 [久保田] 使い勝手の改善を行う（角度をいい感じに調整する）
// 2019/05/23 [久保田] 特定のオブジェクトのみに配置できるように修正
// 2019/05/24 [久保田] Ctrlキーを押しながらクリックをした場合、点を置かなくする(失敗)
// 2019/05/29 [久保田] 新しくアンカーを生成したとき、そのアンカーのハンドルの長さをいい感じにする
// 
// TODO：
//
//────────────────────────────────────────────
...
public class LineCreater : MonoBehaviour
{
	...
	Vector3[] GetLinePositions()
	{
		...
		//Debug.Log(ankers[ankers.Length - 2].AnkerPos);
		return positions;
	}
