...
//────────────────────────────────────────────
// 更新履歴：
// 2019/03/08 [杉山] クラス作成。LineRendererを使い線をつないでいく
// 2019/03/08 [杉山] 実行中ではなくエディター上で動くようにする
// 2019/03/08 [杉山] マウスの左クリックでオブジェクトを置けるように修正　←撤廃。editモードでマウス入力が受け付けられない。
// 2019/03/11 [杉山] マウスの左クリックでオブジェクトを置くための機能を発見。実装を行う。
// 2019/03/11 [杉山] これ以降の実装のためのタスクが多いためマウスでの操作は一旦保留する。
// 2019/04/16 [杉山] 4点以上置かれたときのプログラムを作成
// 2019/04/16 [杉山] 二次ベジェ曲線から三次ベジェ曲線に修正
// 2019/04/16 [杉山] 新たに点が置かれたときに、ハンドルを生成させる
// 2019/04/19 [杉山] アンカークラスを組み込む。
// 2019/05/08 [杉山] 親子関係の中で新たにアンカーが追加されたとき、自動的に追加する。
// 2019/05/09 [杉山] 次のアンカーが生成されたとき、自動的にいい感じの角度に変える
// 2019/05/09 [杉山] クリックしたときクリックされた場所にアンカーを配置する。
// 2019/05/09 [杉山] 実行と同時に一定区間ごとにオブジェクトを置く
// 2019/05/09 [杉山] アタッチされているはずのものが存在しない場合、警告文を出す
// 2019/05/13 [杉山] エディタ上のみで更新処理を行い、それ以外では処理を行わないようにする
// 2019/05/13 [杉山] 角度調整機能を一部調整
// 2019/05/13 [杉山] 長さに応じて間隔を自動調整する(失敗した)
// 2019/05/22 [杉山] 使い勝手の改善を行う（角度をいい感じに調整する）
// 2019/05/23 [杉山] 特定のオブジェクトのみに配置できるように修正
// 2019/05/24 [杉山] Ctrlキーを押しながらクリックをした場合、点を置かなくする(失敗)
// 2019/05/29 [杉山] 新しくアンカーを生成したとき、そのアンカーのハンドルの長さをいい感じにする
// 
// TODO：
//
//────────────────────────────────────────────
